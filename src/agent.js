...
function playAlertSound() {
  ...
  try {
    ...
    // Sequência: 3 bipes em tom crescente — estilo alerta profissional
    const notes = [
...
function startAgent() {
  ...
  log('🏆 King Under Copa do Mundo iniciado!', 'success');
...
async function fetchLiveGames() {
  ...
  try {
    ...
    if (res.status === 401) { log('❌ API Key inválida', 'warn'); return []; }
    if (res.status === 429) { log('⚠ Limite de requisições — usando demo', 'warn'); return generateDemoGames(); }
    if (!res.ok)            { log(`⚠ Erro ${res.status} — usando demo`, 'warn'); return generateDemoGames(); }
    ...
    return matches.map(m => ({
      id:        m.id.toString(),
      home:      m.homeTeam?.shortName || m.homeTeam?.name || 'Time A',
      homeFull:  m.homeTeam?.name || 'Time A',
      homeFlag:  getFlag(m.area?.name),
      away:      m.awayTeam?.shortName || m.awayTeam?.name || 'Time B',
      awayFull:  m.awayTeam?.name || 'Time B',
      awayFlag:  getFlag(m.area?.name),
      league:    m.competition?.name || 'Copa',
      country:   m.area?.name || '',
      minute:    m.minute || 0,
      homeGoals: m.score?.halfTime?.home ?? m.score?.fullTime?.home ?? 0,
      awayGoals: m.score?.halfTime?.away ?? m.score?.fullTime?.away ?? 0,
      isDemo:    false,
    }));
  } catch (err) {
    ...
    <div class="card-body">
      <div class="teams-row">
        ...
        <div class="score-center">
          ...
          <div class="halftime-label">1º TEMPO</div>
        </div>
        <div class="team">
...
async function runScan() {
  ...
  log(`📋 ${games.length} jogos ao vivo`, 'system');
